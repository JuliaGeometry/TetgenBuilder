...
typedef struct {
  ...
  int *tetrahedronlist;
  float *tetrahedronattributelist;
  float *tetrahedronvolumelist;
  int *neighborlist;
  int numberoftetrahedra;
  int numberofcorners;
  int numberoftetrahedronattributes;
  ...
  float *facetconstraintlist;
  int numberoffacetconstraints;

  float *segmentconstraintlist;
  int numberofsegmentconstraints;
...
void copy_tetio(tetgenio* in, TetGenIOf32* out){
    out->firstnumber = in->firstnumber;
    out->mesh_dim = in->mesh_dim;
...
void copy_tetio(TetGenIOf32* in, tetgenio* out){
    out->firstnumber = in->firstnumber;
    out->mesh_dim = in->mesh_dim;
...
extern "C" {
  ...
  TetGenIOf32 tetrahedralizef32(TetGenIOf32 jl_in, char* command){
    tetgenio in, out;
    copy_tetio(&jl_in, &in);
    tetrahedralize(command, &in, &out);
    TetGenIOf32 jl_out;
